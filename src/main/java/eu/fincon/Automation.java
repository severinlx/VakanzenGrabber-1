...
public class Automation {
    ...
    {
        // Suche durchführen
        WebElement searchField = element_Holen(driver, IdentifierType.Xpath, "/html/body/div[2]/div[1]/section[1]/div[1]/article/div/form/div/input[3]");
        element_Setzen(searchField, Suchkriterium);
        WebElement searchButton = element_Holen(driver, IdentifierType.Xpath, "/html/body/div[2]/div[1]/section[1]/div[1]/article/div/form/div/span/button");
    ...
    public static void freelance_suchliste_Durchlaufen(WebDriver driver)
    {
        ...
        do {
            List<WebElement> Sucheinträge = elemente_Holen(driver, IdentifierType.Xpath, "//div[@class='project-list']/*");
            ...
            String pageIdentifier = element_Holen(driver, IdentifierType.Xpath, "//div[@id='pagination']/p").getText();
            sucheinträge_Bearbeiten(driver, Sucheinträge, pagecount);
            // Button für die nächste Seite ermitteln
            nextPageButton = element_Holen(driver, IdentifierType.Xpath, "//a[@aria-label='Next']");
            ...
            {
                element_Klicken(driver, nextPageButton);
                // Seiten zählens
                pagecount += 1;
                int waitcount = 0;
                ...
                while (pageIdentifier == element_Holen(driver, IdentifierType.Xpath, "//div[@id='pagination']/p").getText() || waitcount > (20*1000));
            }
        }
    ...
    public static void sucheinträge_Bearbeiten(WebDriver driver, List<WebElement> sucheinträge, int pagecount) {
        ...
        while (count < sucheinträge.size())
        {
            ...
            // Webelement öffnen
            WebElement ueberschrift = element_Holen(driver, we, IdentifierType.Xpath,"//h3/a");
            element_Klicken(driver, ueberschrift);
            ...
            vorherige_Seite(driver);
            count ++;
            // aufgrund des wechseln zur vorherigen Seite muss die Liste neu erstellt werden
            sucheinträge = elemente_Holen(driver, IdentifierType.Xpath, "//div[@class='project-list']/*");
        }
    }
    ...
    private static Datensatz inserat_Sichern(WebDriver driver) {
        ...
        String Beschreibung = "";
        //Überschrift
        WebElement ÜberschritftElement = element_Holen(driver, IdentifierType.Xpath, "//h1/b");
        if (ÜberschritftElement != null) {
            Überschrift = ÜberschritftElement.getText();
        }
        //Firmenname
        WebElement FirmennameElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='panel-body project-header panel-white']/p[1]");
        if (FirmennameElement != null) {
            Firmenname = FirmennameElement.getText();
        }
        //Datum Von
        WebElement DatumVonElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='project-right-content']//div[@class='col-md-6 details-box'][1]/p[1]");
        if (DatumVonElement != null) {
            DatumVon = DatumVonElement.getText();
        }
        //Datum Bis
        WebElement DatumBisElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='project-right-content']//div[@class='col-md-6 details-box'][1]/p[2]");
        if (DatumBisElement != null) {
            DatumBis = DatumBisElement.getText();
        }
        //Ort
        WebElement OrtElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='project-right-content']//div[@class='col-md-6 details-box'][1]/p[3]");
        if (OrtElement != null) {
            Ort = OrtElement.getText();
        }
        //Bezahlung
        WebElement BezahlungElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='project-right-content']//div[@class='col-md-6 details-box'][2]/p[1]");
        if (BezahlungElement != null) {
            Bezahlung = BezahlungElement.getText();
        }
        // Beschreibung
        WebElement BeschreibungElement = element_Holen(driver, IdentifierType.Xpath, "//div[@class='panel-body']");
